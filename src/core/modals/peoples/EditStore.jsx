...
const EditStore = (p) => {
    ...
    useEffect(() => {
        if (posts?.length > 0) {
            ...
            setNames(temp);
            if (p.updateMode) {
                const res = posts.find((i) => i.clientId === Number(p.invId));
                setClientName(res.clientName);
                setFormData({ ...formData, name: res.clientName, imageName: res.imageName, email: res.email, contact: res.contact, address: res.address, city: res.city, country: res.country });
                if (res.imageName) {
    ...
    // loginUser?.userId
    const [formData, setFormData] = useState({ name: "", imageName: "", email: "", contact: "", address: "", city: "", country: "", createdBy: 1 });
    ...
    const validate = () => {
        ...
        else if (nameRef.current.value !== clientName && names.includes(nameRef.current.value)) {
    ...
    const handleInsert = async (e) => {
        e.preventDefault();
        if (validate()) {
            let path = formData.imageName;
            if (getIsImageChange)
                path = await uploadImage(getImgFile);
            dispatch(updateClient(p.invId, formData, path));
    ...
    return (
        <div>
            <div className="modal fade" id="edit-store" onClick={handleModalClose}>
                                        ...
                                        <div className="row">
                                            <div className="col-lg-12">
                                                <div className="mb-3 input-blocks">
                                                    ...
                                                    <input type="text" className={"form-control " + (errors.nameErr ? "is-invalid" : "")} ref={nameRef} value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                                            ...
                                            <div className="col-lg-6">
                                                <div className="mb-3 mb-0">
                                                    ...
                                                    <input type="text" className="form-control" value={formData.country} onChange={(e) => setFormData({ ...formData, country: e.target.value })} style={{ marginTop: "-9px" }} />
                                                </div>
                                            </div>
                                        </div>
